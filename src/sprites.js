...
const SHAPES = {
  royalslime: (p, id) => `
    <ellipse cx="50" cy="66" rx="38" ry="26" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
  ...
  demonbull: (p, id) => `
    <ellipse cx="52" cy="62" rx="33" ry="24" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
  ...
  violetbeast: (p, id) => `
    ...
    <ellipse cx="52" cy="64" rx="34" ry="23" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
  ...
  angelwing: (p, id) => `
    ...
    <ellipse cx="50" cy="40" rx="18" ry="17" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
  ...
  toxinblob: (p, id) => `
    <ellipse cx="50" cy="64" rx="33" ry="26" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
    <path d="M17 64 q0 -26 33 -26 q33 0 33 26 Z" fill="${p.light}" opacity=".4"/>
    ${[[26,84],[42,88],[60,87],[74,83]].map(([x,y])=>
      `<ellipse cx="${x}" cy="${y}" rx="4" ry="6" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}" opacity=".8"/>`).join('')}
  ...
  crimworm: (p, id) => `
    ${[[22,80,9],[36,74,10],[50,66,11]].map(([x,y,r])=>
      `<circle cx="${x}" cy="${y}" r="${r}" fill="${p.dark}"/>
       <circle cx="${x}" cy="${y-1}" r="${r-3}" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>`).join('')}
    <circle cx="68" cy="52" r="16" fill="${p.dark}"/>
    <circle cx="68" cy="51" r="13" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
  ...
  frostsquid: (p, id) => `
    <path d="M50 16 q24 18 24 44 q0 12 -24 12 q-24 0 -24 -12 q0 -26 24 -44 Z" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
  ...
  embermoth: (p, id) => `
    <path d="M34 46 q-30 -20 -30 6 q0 20 28 16 Z" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
    <path d="M66 46 q30 -20 30 6 q0 20 -28 16 Z" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
    ...
    <ellipse cx="50" cy="54" rx="7" ry="16" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
  ...
  shadowbat: (p, id) => `
    <path d="M28 50 q-26 -18 -26 2 q0 8 6 12 q4 -6 8 -2 q2 8 8 6 q2 -8 4 -18 Z" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
    <path d="M72 50 q26 -18 26 2 q0 8 -6 12 q-4 -6 -8 -2 q-2 8 -8 6 q-2 -8 -4 -18 Z" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
    <path d="M28 50 q-18 -12 -18 2 q0 6 5 9 Z" fill="${p.light}" opacity=".4"/>
    <path d="M72 50 q18 -12 18 2 q0 6 -5 9 Z" fill="${p.light}" opacity=".4"/>
    <path d="M38 32 l3 -14 l7 12 Z" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
    <path d="M62 32 l-3 -14 l-7 12 Z" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
    <ellipse cx="50" cy="56" rx="22" ry="23" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
  ...
  steelcrab: (p, id) => `
    <path d="M22 50 q-18 -8 -20 -22 q12 -5 19 5 q7 10 5 17 Z" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}" stroke="${p.dark}" stroke-width="1.5"/>
    <path d="M78 50 q18 -8 20 -22 q-12 -5 -19 5 q-7 10 -5 17 Z" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}" stroke="${p.dark}" stroke-width="1.5"/>
    <ellipse cx="50" cy="62" rx="30" ry="22" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
  ...
  sandbug: (p, id) => `
    ...
    <ellipse cx="50" cy="60" rx="32" ry="26" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
  ...
  deepfish: (p, id) => `
    ...
    <path d="M20 62 l-11 -10 v20 Z" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
    <ellipse cx="54" cy="62" rx="33" ry="23" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
  ...
  voidwisp: (p, id) => `
    <path d="M22 60 q0 -36 28 -36 q28 0 28 36 v22
             q-7 -9 -14 0 q-7 9 -14 0 q-7 -9 -14 0 q-7 9 -14 0 Z" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
  ...
  spikelizard: (p, id) => `
    ...
    <ellipse cx="52" cy="64" rx="32" ry="22" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}"/>
...
const GUARD_SHAPES = {
  shield: (p, id) => `
    <path d="M50 12 l32 11 v28 q0 28 -32 37 q-32 -9 -32 -37 v-28 Z" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}" stroke="${p.trim}" stroke-width="2"/>
  ...
  scanner: (p, id) => `
    <rect x="18" y="34" width="64" height="44" rx="9" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}" stroke="${p.trim}" stroke-width="2"/>
  ...
  turret: (p, id) => `
    ...
    <ellipse cx="48" cy="56" rx="25" ry="23" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}" stroke="${p.trim}" stroke-width="2"/>
  ...
  sentinel: (p, id) => `
    <path d="M50 10 l28 15 v30 q0 24 -28 33 q-28 -9 -28 -33 v-30 Z" fill="${p.body.startsWith? (p.body.startsWith('url')||p.body==='transparent' ? p.body : `url(#bodyG${id})`) : p.body}" stroke="${p.trim}" stroke-width="2"/>
...
// Shared defs injected into every creature: soft body gradient, rim
// light, and an outline filter. This is what lifts the art from "flat
// vector blob" to something that reads as a lit, solid creature.
function creatureDefs(id, p) {
  return `
  <defs>
    <radialGradient id="bodyG${id}" cx="38%" cy="28%" r="78%">
      <stop offset="0%"   stop-color="${p.light}"/>
      <stop offset="48%"  stop-color="${p.body}"/>
      <stop offset="100%" stop-color="${p.dark}"/>
    </radialGradient>
    <linearGradient id="rimG${id}" x1="0" y1="1" x2="0.4" y2="0">
      <stop offset="0%"   stop-color="${p.accent}" stop-opacity=".55"/>
    ...
    <filter id="ink${id}" x="-25%" y="-25%" width="150%" height="150%">
      ...
      <feFlood flood-color="#0d0818" result="c"/>
...
export function creatureSVG(shape, paletteKey, opts = {}) {
  ...
  return `<svg class="${cls}" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"
    ...
    <ellipse cx="50" cy="95" rx="26" ry="4.5" fill="#000" opacity=".42" filter="url(#soft${id})"/>
    <g class="cr-body" filter="url(#ink${id})">
      ${builder(p, id)}
      <!-- rim light along the top edge -->
      <g class="cr-rim" opacity=".9" style="mix-blend-mode:screen;pointer-events:none">
        ${builder({ ...p, body:`url(#rimG${id})`, dark:'transparent',
                    light:'transparent', accent:'transparent', trim:'transparent' }, id)}
      </g>
    </g>
  </svg>`;
}
